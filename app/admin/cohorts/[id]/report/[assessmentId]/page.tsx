    ...
    return (
      <div className="py-6 space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-[color:var(--color-chessboard-ivory)]" style={{ fontFamily: 'var(--font-display)' }}>
            User Report
          </h1>
        </div>
        <StoneCard>
          <p className="text-center text-[color:var(--color-chessboard-smoke)]" style={{ fontFamily: 'var(--font-body, serif)' }}>
            {error}
          </p>
        </StoneCard>
      </div>
    ...
    <div className="py-6 space-y-6">
        ...
        <div className="flex-1">
          <h1
            ...
            style={{
              ...
              background: 'linear-gradient(135deg, var(--color-chessboard-gold), var(--color-chessboard-gold-bright))',
        ...
        <motion.div variants={staggerItem}>
          <StoneCard padding="none">
            ...
            <div className="p-6 space-y-4">
              <div>
                <span className="text-[10px] uppercase tracking-[0.14em] text-[color:var(--color-chessboard-smoke)]" style={{ fontFamily: 'var(--font-display)' }}>
                  Archetype Profile
                </span>
                <p className="text-lg font-medium text-[color:var(--color-chessboard-gold)] mt-1" style={{ fontFamily: 'var(--font-display)' }}>
              ...
              <div>
                ...
                <p className="text-lg font-medium text-[color:var(--color-chessboard-ivory)] mt-1" style={{ fontFamily: 'var(--font-display)' }}>
                  {report.organizationalRole || 'N/A'}
                </p>
              </div>
              {report.archetypeNarrative && (
                <div className="mt-4 p-4 bg-[color:var(--color-chessboard-gold)]/[0.04] border border-[color:var(--color-chessboard-gold)]/10 rounded-lg">
                  <p className="text-sm italic text-[color:var(--color-chessboard-smoke)] break-words" style={{ fontFamily: 'var(--font-body, serif)' }}>
                    {report.archetypeNarrative}
                  </p>
                </div>
              )}
      ...
      {report.competencyRanking && (
        <StoneCard padding="none">
          <div className="flex items-center gap-2 px-6 py-4 border-b border-[color:var(--color-chessboard-ash)]/20">
            <Award className="h-5 w-5 text-[color:var(--color-chessboard-gold)]" />
            <h2 className="text-xs uppercase tracking-[0.16em] text-[color:var(--color-chessboard-smoke)]" style={{ fontFamily: 'var(--font-display)' }}>
          ...
          <div className="p-6 space-y-4">
            {(report.competencyRanking as unknown as AdminRankedCompetency[]).map((comp) => (
              <div
                key={comp.code}
                className="p-4 border border-[color:var(--color-chessboard-ash)]/15 rounded-lg bg-[color:var(--color-chessboard-gold)]/[0.02]"
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold text-base text-[color:var(--color-chessboard-ivory)]" style={{ fontFamily: 'var(--font-display)' }}>
                    {comp.name}
                  </h3>
                ...
                <p className="text-[10px] uppercase tracking-[0.1em] text-[color:var(--color-chessboard-smoke)] mb-4" style={{ fontFamily: 'var(--font-display)' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-3">
                  {comp.strengths && comp.strengths.length > 0 && (
                    <div>
                      <span className="font-semibold text-[color:var(--color-chessboard-verdant)] block mb-1 text-xs" style={{ fontFamily: 'var(--font-display)' }}>
                        Strengths
                      </span>
                      <ul className="list-disc pl-4 space-y-1 text-[color:var(--color-chessboard-smoke)]" style={{ fontFamily: 'var(--font-body, serif)' }}>
                        {comp.strengths.map((s: string, i: number) => (
                          <li key={i} className="text-sm marker:text-[color:var(--color-chessboard-verdant)]/40">{s}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  {comp.weaknesses && comp.weaknesses.length > 0 && (
                    <div>
                      <span className="font-semibold text-[color:var(--color-chessboard-crimson)] block mb-1 text-xs" style={{ fontFamily: 'var(--font-display)' }}>
                        Areas for Growth
                      </span>
                      <ul className="list-disc pl-4 space-y-1 text-[color:var(--color-chessboard-smoke)]" style={{ fontFamily: 'var(--font-body, serif)' }}>
                        {comp.weaknesses.map((w: string, i: number) => (
                          <li key={i} className="text-sm marker:text-[color:var(--color-chessboard-crimson)]/40">{w}</li>
                        ))}
                      </ul>
                    </div>
                  )}
      ...
      {report.detailedAnalysis && (
        <StoneCard padding="none">
          ...
          <div className="p-6">
            <p className="whitespace-pre-line text-sm text-[color:var(--color-chessboard-smoke)]" style={{ fontFamily: 'var(--font-body, serif)' }}>
              {report.detailedAnalysis}
            </p>
          </div>
        </StoneCard>
      ...
      {engagement && Object.keys(engagement).length > 0 && (
        <StoneCard padding="none">
          <div className="px-6 py-4 border-b border-[color:var(--color-chessboard-ash)]/20">
            ...
            <p className="text-xs text-[color:var(--color-chessboard-smoke)] mt-1" style={{ fontFamily: 'var(--font-body, serif)' }}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[color:var(--color-chessboard-ash)]/20">
                  {['Phase', 'Spam %', 'Burst', 'Floor', 'Selections'].map((h, i) => (
                    <th
                      key={h}
                      className={`py-2 px-2 font-medium text-[color:var(--color-chessboard-smoke)] text-[10px] uppercase tracking-[0.14em] ${i >= 1 ? 'text-right' : 'text-left'}`}
              ...
              <tbody>
                {Object.entries(engagement).map(([stage, e]) => {
                  ...
                  return (
                    <tr key={stage} className="border-b border-[color:var(--color-chessboard-ash)]/10 last:border-0">
                      <td className="py-2 px-2 font-mono text-[10px] text-[color:var(--color-chessboard-smoke)]">
                        {stage.replace('STAGE_', '').replace(/_/g, ' ')}
                      </td>
                      <td className={`py-2 px-2 text-right font-mono ${color}`}>{pct}%</td>
                      <td className="py-2 px-2 text-right font-mono text-[color:var(--color-chessboard-smoke)]">{e?.burstEvents ?? 0}</td>
                      <td className="py-2 px-2 text-right font-mono text-[color:var(--color-chessboard-smoke)]">{e?.floorEvents ?? 0}</td>
                      <td className="py-2 px-2 text-right font-mono text-[color:var(--color-chessboard-smoke)]">{e?.totalSelections ?? 0}</td>
                    </tr>
                  )
                })}
              </tbody>
